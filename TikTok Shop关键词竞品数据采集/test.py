...
class TikTokPage:
    ...
    def main(self):
        ...
        time.sleep(1)
        current_keyword = (self.page.ele('x://input[@aria-label="Search"]', timeout=10).attr('value') or '').strip()
        ...
        print(f'共 {len(links)} 条链接, 相关关键词商品链接共 {len(links)-10} 条')
        ...
        for i, link in enumerate(links[:-10], 1):
            ...
            try:
                ...
                # 获取销售店铺
                pl = tab.ele('x://a[contains(text(), "销售店铺")]', timeout=10)
                if pl:
                    shop_name = re.sub(r'^销售店铺\s*', '', pl.text.strip())
                    item_data['销售店铺'] = shop_name
                ...
                # 获取商品话题标签
                biaoqian = re.findall(r'#\S+', shop_title)
                if biaoqian:
                    shop_biaoqian = biaoqian
                    item_data['商品话题标签'] = shop_biaoqian
                    print(f'商品标签: {shop_biaoqian}')

                # 获取售卖单价
                pice = tab.ele('x://span[contains(@class, "items-baseline")]')
                shop_pice = pice.text.strip() if pice else ''
                item_data['售卖单价'] = shop_pice
                print("售卖单价: "+shop_pice)
                # 获取运费（Free shipping=0 美元包邮）
                free_shipping = tab.ele('x://span[contains(text(), "免费运送")]', timeout=10)
                if free_shipping:
                    item_data['运费'] = get_text(free_shipping)
                else:
                    order_shipping = tab.ele('x://span[contains(text(), "此订单运费为")]', timeout=10)
                    if order_shipping:
                        item_data['运费'] = get_text(order_shipping)
                # 产品星级
                review_section = tab.ele('x://div[@id="pdp-review-section"]', timeout=10)
                if review_section:
                    review_section.scroll.to_see(center=True)
                    time.sleep(1)
                rating = tab.ele('x://div/span[contains(@class, "H2-Semibold mr-2")]')
                # rating.scroll.to_see(center=True)
                item_data['产品星级'] = rating.text
                time.sleep(1)
                # 获取评价总数
                review_total = tab.ele('x://div[contains(@class, "H2-Semibold")]')
                item_data['评价总数'] = review_total.text
                time.sleep(1)
                # 五星 / 四星分项评价数量
                five_star_count = tab.ele('x://div[div/div[text()="5"]]//div[contains(@class,"H3-Regular")]')
                item_data['五星评价数量'] = five_star_count.text
                time.sleep(1)
                four_star_count = tab.ele('x://div[div/div[text()="4"]]//div[contains(@class,"H3-Regular")]')
                item_data['四星评价数量'] = four_star_count.text
                time.sleep(1)
                # 累计已售件数
                sold_count = tab.ele('x://span[contains(@class,"H3-Regular")][contains(.,"已售")]')
                item_data['累计已售件数'] = sold_count.text

                # 全系列变体
                if tab.ele('x://span[text() = "Scent"]'):
                    bainti = tab.eles('x://div[contains(@class,"border-solid")]')
                    bianti_count = len(bainti)
                    print(bianti_count)
                else:
                    print("该商品无任何变体信息")
