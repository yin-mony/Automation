...
class TestPage:
    ...
    def main(self):
        ...
        for index, ip in enumerate(self.ip):
            ...
            for data in self.data:
                if page.ele(f'x://span[text()="{data}"]'):
                    print(f'{data}已是当前站点,跳过切换')
                else:
                    page.ele('x://span[text()="KORCCI LLC"]').click()
                    time.sleep(0.78)
                    page.ele('x://*[text()="See all"][1]').click()
                    time.sleep(0.78)
                    # page.ele('x://*[text()="查看所有"]').click()
                    # time.sleep(5)
                ...
                if data == 'United States':
                    ...
                    time.sleep(3)
                    # 年份
                    # dropdown_year = page.ele('x://div[@class="date-range-item"][2]/kat-dropdown')
                    # dropdown_year.click()
                    # dropdown_year.ele(f'x://kat-option//div[text()="{year}"]',timeout=20).click()
                    # time.sleep(3)

                    # page.ele('x://kat-button[@label="Request Report"]', timeout=20).click()
                    # time.sleep(1.5)
    ...
    def run(self):
        ...
        # self.wait_for_port(9222)
        # sp.YidekeLogin()
        # time.sleep(3)
        # # 访问指定的店铺
        # self.visit_store(self.ip,port=9222)
        # # 3. 8888 负责接管新打开的 KOR 店铺浏览器
        # store_port = self.port  # 配置里的 8888
        # self.wait_for_port(store_port, timeout=90)
        # self.main(store_port)
        self.main()

        # page = ChromiumPage("127.0.0.1:" + str(8888))  # 接管浏览器
        # page.ele('x://div/kat-radiobutton[@label="Month"]').click(by_js=True)
        #
        # dropdown_month = page.ele('x://div/div/kat-dropdown')
        # dropdown_month.click()
        # dropdown_month.shadow_root(f'x://kat-option//div[text()="June"]').click()
        # print(dropdown_month.html)
        # dropdown_stores = page.ele('x://form/div[1]/kat-dropdown')
        # dropdown_stores.click()
        # dropdown_stores.shadow_root('x://kat-option[@value="ALL_STORES"]').click()
        # time.sleep(1)
        # dropdown_account = page.ele('x://form/div[2]/kat-dropdown')
        # dropdown_account.click()
        # dropdown_account.shadow_root('x://kat-option[@value="ALL"]').click()
        # time.sleep(1)
        # dropdown_report = page.ele('x://form/div[3]/kat-dropdown')
        # dropdown_report.click()
        # dropdown_report.shadow_root('x://kat-option[@value="SELLER_SUMMARY_DATE_RANGE"]').click()
        # time.sleep(1)
        # page.ele('x://div/kat-radiobutton[@label="Month"]').click(by_js=True)
        # print(dropdown.html)
