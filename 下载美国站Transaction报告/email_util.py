...
PROJECT_NAME = "下载美国站Transaction报告"
...
def deliver_outputs(config, file_paths, subject=None, content=None):
    ...
    if not sender or not auth_code:
        raise ValueError(
            "发送邮件时必须配置 sender_email/smtp_auth_code，"
            "或设置 SMTP_SENDER/SMTP_AUTH_CODE 环境变量"
        )

    if not paths:
        print("未找到可发送的报告文件，跳过邮件")
        return False
    ...
    return send_email_with_files(
        ...
        content=content or f"自动化_{PROJECT_NAME}下载的文件",
        file_paths=paths,
    )
