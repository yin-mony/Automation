"""
下载美国站 Transaction 报告 — Tkinter GUI 入口。

从界面收集 config，后台线程调用 main.TestPage.run()，
...
class AutomationGui:
    """主窗口：易得客配置、开始运行、日志展示（站点固定美国）。"""

    def __init__(self, root):
        self.root = root
        self.root.title("下载美国站 Transaction 报告")
    ...
    def _build_ui(self):
        ...
        ttk.Label(form, text="站点").grid(row=4, column=0, sticky=tk.W, padx=(0, 8), pady=6)
        ttk.Label(form, text="United States（固定）").grid(row=4, column=1, sticky=tk.W, pady=6)
        ...
        self.log_text = scrolledtext.ScrolledText(log_box, height=16, wrap=tk.WORD)
