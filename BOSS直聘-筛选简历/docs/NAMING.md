...
| 文件名 | 类名 | 说明 |
|--------|------|------|
| `auto.py` | `BossAuto` | 浏览器自动化 |
| `db.py` | `BossDb` | 数据库操作 |
| `server.py` | `WebServer` | Web 服务 |
| `service.py` | `TaskService` | 任务调度 |
| `worker.py` | `TaskWorker` | 任务执行线程 |
| `parse.py` | `ResumeParse` | 简历解析 |
| `match.py` | `ResumeMatch` | 简历匹配 |
| `hub.py` | `SocketHub` | WebSocket 集线器 |
| `http.py` | `BossHttp` | HTTP 静态与导出 |
...
class BossAuto:
    def __init__(self):
        ...
        # 默认年龄范围
        self.ageFilter = {"最低": 16, "最高": "不限"}
        # 浏览器 API 地址
        self.bitUrl = "http://127.0.0.1:54345"
        self.adsUrl = "http://127.0.0.1:50325"
        # 运行状态
        self.stopFlag = False
        self.page = None
```
...
| 模块职责 | 推荐文件名 | 主类名 | 避免 |
|----------|-----------|--------|------|
| BOSS 浏览器自动化 | `auto.py` | `BossAuto` | `boss_auto.py`、`browser_utils.py` |
| 数据库操作 | `db.py` | `BossDb` | `automation_db.py`、`db_helper.py` |
| Web/WS 服务 | `server.py` | `WebServer` | `web_server.py`、`http_manager.py` |
| 任务调度 | `service.py` | `TaskService` | `task_service.py`、`automation_service_handler.py` |
| 任务线程 | `worker.py` | `TaskWorker` | `task_worker.py` |
| 简历解析 | `parse.py` | `ResumeParse` | `resume_parser.py` |
| 简历匹配 | `match.py` | `ResumeMatch` | `resume_matcher.py` |
| WebSocket | `hub.py` | `SocketHub` | `socket_hub.py` |
| HTTP 导出 | `http.py` | `BossHttp` | `boss_http.py` |
| 前端逻辑 | `app.js` | — | `main_application_logic.js` |
| 样式 | `styles.css` | — | `global_stylesheet.css` |
...
rule.delete
task.submitChat
task.submitRecommend
worker.pause
worker.stop
db.refresh
```

> 相比旧版 `keyword.create`，新版统一缩短为 `rule.create`，保持简短。
